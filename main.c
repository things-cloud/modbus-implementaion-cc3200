...
#define APP_NAME                        "The ThingsCloud"
...
void construct_post_buffer(int l,int ind)
{
    ...
    for (i = start_val;i<end_val;i++)
    {
       temp = Adress[i]-30001;
       temp_index = i;
       Query_set[0]= slave_list[ind];
       Query_set[1] = 0x04;
       Query_set[3] = (char)(temp&0x000000FF);
       Query_set[2] = (char)((temp&0x0000FF00)>>8);
       Query_set[4] = 0x00;
       if((Adress[i]==30002)||(Adress[i]==30004)||(Adress[i]==30006)||(Adress[i]==30038)||(Adress[i]==30040))
       Query_set[5] = 0x02;
       else
       Query_set[5] = 0x01;
       lRetVal = send_validate(Query_set);
       if(lRetVal!=0)
       {
           UART_PRINT("Slave doesn't exist");
           slave_list[0] = 0;
       }
       temp = 0;
       if(i==(end_val-1))
    	   sprintf(temp_str,"\n\"%s\":%d",name_list[i],result);
       else
    	   sprintf(temp_str,"\n\"%s\":%d,",name_list[i],result);
...
void append_checksum(char* Buf)
{
    int checksum;

    checksum = ModRTU_CRC(Buf,6);
    *(Buf+6) = (char)((checksum&0x0000FF00)>>8);
    *(Buf+7) = (char)(checksum&0x000000FF);
}
void Recv_response()
{
   int i =0;
...
int extract_checksum(char *buf,int len)
{
    int checksum ;
    checksum = *(buf+len-2);
    checksum<<=8;
    checksum = checksum|(*(buf+len-1));
    return checksum;
}
void send_cmd(char term,char *Buf,int len)
{
    int i;
    char c;
    if(term == Debug)
    ...
    }else
    {
        while(UARTCharsAvail(CONSOLE1))
          {
              c = UARTCharGetNonBlocking(CONSOLE1);
          }
        for (i=0;i<len;i++)
                {
                    MAP_UARTCharPut(CONSOLE1,Buf[i]);
                }
        while(MAP_UARTBusy(UARTA1_BASE));
    }

}
int send_validate(char* Buf)
{
    static int i=0;
    int check_sum,calc_checksum,ret_val;
     result = 0;
    ...
    delay(100);
    send_cmd(Slave,Buf,8);

    delay(10000);
    check_sum = extract_checksum(recv_buffer,uartRxLength);
    calc_checksum = ModRTU_CRC(recv_buffer,uartRxLength-2);
    if(check_sum == calc_checksum)
    {
        if(recv_buffer[0] == Buf[0] && recv_buffer[1]==Buf[1])
        {
            GPIO_IF_LedOn(MCU_GREEN_LED_GPIO);
            if((Adress[temp_index]==30002)||(Adress[temp_index]==30004)||(Adress[temp_index]==30006)||(Adress[temp_index]==30038)||(Adress[temp_index]==30040))
            {
            	result |= recv_buffer[3];
            	result = ((result<<8)|recv_buffer[4]);
            	result = ((result<<8)|recv_buffer[5]);
            	result = ((result<<8)|recv_buffer[6]);
            }
            else
            {	result |= recv_buffer[3];
                result = ((result<<8)|recv_buffer[4]);
...
void  Get_slave_ids()
{
    ...
    int ret_val,s;
    char Test_Query[8] = {0x03,0x04,0x00,0x00,0x00,0x01};
   s=0;
  for(i = 1;i<10;i++)
    {
        Test_Query[0] = (char)i;
        ret_val = send_validate(Test_Query);
#ifdef WATCHDOG
watchdogack();
#endif
        if(ret_val == 0)
